...
//
// An 'inplace T' must contain everything needed to construct the 'T' value. In the example above,
// the effect is similar to if 'create_foo' returned a closure to be evaluated later:
...
// what is specified, errors can be reported reliably and clearly.
//
// Scope of deferral
// =================
//
// When a compile-time expression is converted from 'T' to 'inplace T', code to construct the 'T'
// value is deferred to the initializer routine. In the simple example above, this conversion was
// caused by the need to convert a 'Foo' to 'inplace Foo' in order to satisfy the return type, so
// only the final values of each field were captured and stored with the initializer, and only
// the actual construction of the complete 'struct Foo' was deferred.
//
...
// The scope of deferral is therefore controllable, explicit, and minimised by default.
//
// Error handling
// ==============
//
// Code that produces an 'inplace T' must complete any error handling which could prevent it from
// returning a valid 'T' value, prior to the creation of an 'inplace T'. The code deferred to the
...
// This constraint does not completely disallow further error handling within the initializer, but
// it does limit that code to outcomes that still produce a valid 'T' value.
//
// Code in the initializer must never unwind, since to do so could leave references pointing to an
// uninitialized value. The compiler must reject the code if it cannot compile the initializer
// without unwinding.
//
// This implies that the compiler must reject any may code in an 'inplace' block that may panic, in
// any configuration where a panic results in an unwind. However, it would be acceptable to allow
// panics in the initializer in a configuration where a panic results in an abort.
//
// Composition
// ===========
//
...
// We apply a rule that when an 'inplace T' value is used as a field in another 'inplace'
...
// ==========================
//
// This proposal provides no support for fallible initialization. However, it does support:
//
// - Fallible allocation.
// - Error handling which produces a Result prior to the final initialization.
// - Composition of error handling by the usual mechanisms, resulting in a compound initializer.
// - Error handling within the deferred initializer which still produces a valid value.
//
// We argue that these benefits are sufficient to justify the proposal without support for fallible
// initialization.
//
// In principle, it might be straightforward to adapt this proposal such that rather than just
// 'inplace T' we have for instance a magic trait 'Inplace<T, E>' in which E is an error type that
// may be potentially returned by the initializer.
//
// However, since there is no way for an assignment to result in an error, the conversion to 'T'
// would require some other operation to be invoked to perform the initialization and optionally
// return an error. Assuming this were a function call, it would need to be passed a pointer to
// uninitialized memory, so would only be usable in an unsafe context. That would be a significant
// step backwards because as it stands, this proposal allows in place initialization with no unsafe
// code, except in places where uninitialized memory is already in use, such as within the
// implementation of a container type.
//
// Supporting fallible initialization would further require every method on a container, or other
// function that accepted an 'inplace T', to be genericised such that it could also return a result
// indicating failure of the initialization. This would reintroduce the API churn problem.
//
// Self-referential structures
// ===========================
//
// With the ability to require that structures be created in place, it becomes possible in
// combination with pinning for self-referential structures to be created safely. Doing so would
// require changes to the borrow checker as well as some syntactic means of self-reference.
//
...
// This would address issues affecting the use of Rust in the Linux kernel, as discussed at:
